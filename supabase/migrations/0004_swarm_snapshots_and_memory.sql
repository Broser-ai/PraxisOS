-- PraxisOS · swarm durability tables used by app code but missing on remote + repo
-- Consumers:
--   lib/swarm/persist.ts        → public.swarm_snapshots
--   agents/memory/swarm-memory.ts → public.swarm_memory (REST upsert)
...
begin
  if not exists (
    ...
    -- Intentional: only service_role / backend writes.
    -- Defense in depth: enable RLS with deny policies for anon/authenticated.
...
-- Optional vector memory for agent recall (embedding may be null when local-only)
