...
5. **WordPress (Hetzner-styret stack i repo):**
   ...
   - `/behandlinger/` bruger shortcodes (API SoT — ingen Planway)
   - Forside CTA → `data-praxis-book` / `https://app.bypilar.dk/t/bypilar/book`
   - Planway purge: ingen `planway.com` i tema/bridge; booking ONLY via app.bypilar.dk
...
| Side «Book Tid» | HTTPS iframe `https://app.bypilar.dk/t/bypilar/book?embed=1` (+ `data-praxis-book`) |
| Staff | `[praxis_klinik]` / «Kom i gang · Klinik» → `https://app.bypilar.dk/login` (ingen produktnavn på kundens site) |
