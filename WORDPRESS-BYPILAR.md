...
- Klinik-app på `https://app.bypilar.dk` / port 3010.
- **Booking ONLY** via `https://app.bypilar.dk/t/bypilar/book` (+ embed `?embed=1`) — **ingen Planway**.
- WordPress-knapper: `data-praxis-book` + script `/embed/v1/bypilar`.
- Staff: «Kom i gang · Klinik» → `https://app.bypilar.dk/login` (ingen produktnavn på kundens site).
