...
import { safeStaffNextPath } from "@/lib/bypilar-host";

type TenantPickItem = { slug: string; role: string };

export type LoginFormProps = {
  /** When true (bypilar customer hosts), hide PraxisOS B2B chrome. */
  whiteLabel: boolean;
  clinicName: string;
  /** Post-login path (validated). Default /dashboard for staff entry. */
  nextPath: string;
};

export default function LoginForm({ whiteLabel, clinicName, nextPath }: LoginFormProps) {
  const r = useRouter();
  ...
  const destination = safeStaffNextPath(nextPath, whiteLabel ? "/dashboard" : "/review");
  ...
  return (
    <div className="min-h-screen bg-paper">
      <div className="grid min-h-screen lg:grid-cols-2">
          ...
          <div className="w-full max-w-[400px]">
            <div className="flex items-center gap-2.5">
              <div className="grid h-8 w-8 place-items-center rounded-[9px] bg-ink text-paper">
                {whiteLabel ? (
                  <span className="text-[14px] font-semibold">{clinicName.charAt(0)}</span>
                ) : (
              ...
              <div className="leading-tight">
                <div className="display text-[17px] font-semibold">
                  {whiteLabel ? clinicName : "PraxisOS"}
                </div>
                <div className="kicker !text-[9.5px]">
                  {whiteLabel ? "Klinik-login · Staff" : "Clinical OS"}
                </div>
              </div>
            </div>
            ...
            {step === "creds" && (
              <>
                <h1 className="display mt-10 text-[28px] font-semibold leading-tight">
                  {whiteLabel ? "Klinik-login" : "Velkommen tilbage."}
                </h1>
                <p className="mt-2 text-[13px] text-muted">
                  {whiteLabel
                    ? "Log ind for at åbne kalender, klienter, journal og scan."
                    : "Log ind for at administrere din klinik."}
                </p>
                  ...
                  <div className="grid grid-cols-2 gap-1.5">
                    {(whiteLabel
                      ? [
                          ...
                          ["sofie@bypilar.dk", "Sofie · Behandler"],
        ...
        {/* Højre · brand-side (PraxisOS B2B only — never on byPilar customer hosts) */}
        {!whiteLabel && (
        ...
        {whiteLabel && (
          <div className="hidden lg:block border-l border-line bg-[#f3ede1] p-12">
            <div className="flex h-full flex-col justify-between">
              <div></div>
              <div>
                <h2 className="display text-[36px] font-semibold leading-[1.05]" style={{ maxWidth: 420 }}>
                  {clinicName} — <em style={{ color: "#8a6a3d" }}>klinik-system</em>
                </h2>
                <p className="mt-4 max-w-[400px] text-[14px] text-muted">
                  Efter login: overblik, kalender, klienter, journal, fod-scan og admin.
                </p>
                <ul className="mt-6 space-y-2 text-[13px] text-muted">
                  {[
                    ...
                    ["Journal", "/journal"],
                    ["Fod-scan", "/scan"],
                    ["Admin", "/admin/packaging"],
                  ].map(([label, href]) => (
                    <li key={href} className="flex gap-2">
                      <span className="text-faint">→</span>
                      <span>{label} <span className="mono text-[11px] text-faint">{href}</span></span>
                    </li>
                  ))}
                </ul>
              </div>
              <div className="text-[11px] text-faint">© {clinicName}</div>
            </div>
          </div>
        )}
      </div>
