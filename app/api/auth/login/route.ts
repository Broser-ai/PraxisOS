...
import { encodeSession, findAccount, SESSION_COOKIE } from "@/lib/auth";
import { authenticateAgainstSupabase } from "@/lib/data/repo";
import {
  getBackoffMs,
  recordAttempt,
  requiresCaptcha,
...
export async function POST(req: Request) {
  ...
  const picked = tenant
    ? acc.tenants.find((t) => t.slug === tenant)
    : acc.tenants[0];
  if (!picked) {
